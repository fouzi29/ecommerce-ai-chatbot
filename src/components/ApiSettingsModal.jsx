...
export function ApiSettingsModal({
  ...
  // Database Integration State & Webhooks (Custom REST API, Orders & Leads)
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div
        className="bg-white border border-purple-200 rounded-2xl max-w-xl w-full p-6 sm:p-8 relative shadow-2xl overflow-hidden animate-slideUp text-slate-900"
      ...
      >
        ...
        <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-extrabold text-slate-900 text-lg tracking-tight">SaaS Client Control Panel</h3>
              <p className="text-slate-600 text-xs">Configure AI Engines, Custom REST Endpoints & WhatsApp Gateway</p>
            </div>
          </div>
        ...
        {/* Navigation Tabs */}
        <div className="flex items-center gap-1.5 mb-6 bg-slate-100 p-1.5 rounded-xl border border-slate-200 shadow-inner">
          <button
            type="button"
        ...
        <form onSubmit={handleSave} className="space-y-5 max-h-[58vh] overflow-y-auto pr-1 text-xs">
            ...
            <>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${
                      provider === "demo" ? "bg-purple-50 border-purple-500 text-purple-900" : "bg-slate-50 border-slate-200 text-slate-600"
                    }`}
                  >
                    ...
                    <span className="text-[10px] text-slate-500 block leading-tight">Zero API key needed</span>
                  </button>
                  ...
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${
                      provider === "openai" ? "bg-emerald-50 border-emerald-500 text-emerald-900" : "bg-slate-50 border-slate-200 text-slate-600"
                    }`}
                  >
                    <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] text-slate-500 block leading-tight">GPT-4o & GPT-4o-mini</span>
                  </button>
                  ...
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${
                      provider === "gemini" ? "bg-cyan-50 border-cyan-500 text-cyan-900" : "bg-slate-50 border-slate-200 text-slate-600"
                    }`}
                  >
                    <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] text-slate-500 block leading-tight">Gemini 2.0 & 1.5 Flash</span>
                  </button>
                </div>
              </div>
              ...
              {provider === "openai" && (
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">OpenAI API Key (sk-...)</label>
                    <input
                      type="password"
                      placeholder="sk-proj-..."
                      value={openAiKey}
              ...
              {provider === "gemini" && (
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">Google Gemini API Key (AIza...)</label>
                    <input
                      type="password"
                      placeholder="AIzaSy..."
                      value={geminiKey}
          ...
          {/* TAB 2: DATABASE & API WEBHOOKS */}
          {activeTab === "database" && (
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                  Select Product Catalog & Webhook Sync Source
                </label>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-3">
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${
                      dbMode === "demo" ? "bg-purple-50 border-purple-500 text-purple-900" : "bg-slate-50 border-slate-200 text-slate-600"
                    }`}
                  >
                    <Layers className="w-4 h-4 text-purple-600 mb-1" />
                    <span className="font-bold text-xs block">Default Catalog</span>
                    <span className="text-[10px] text-slate-500">Built-in sample items</span>
                  </button>
                  ...
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${
                      dbMode === "custom_api" ? "bg-cyan-50 border-cyan-500 text-cyan-900" : "bg-slate-50 border-slate-200 text-slate-600"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${
                      dbMode === "shopify" ? "bg-emerald-50 border-emerald-500 text-emerald-900" : "bg-slate-50 border-slate-200 text-slate-600"
                    }`}
                  >
              ...
              {dbMode === "custom_api" && (
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                  <h4 className="text-xs font-bold text-cyan-700 flex items-center gap-1.5">
                    <Server className="w-3.5 h-3.5" />
                    <span>Custom Backend Endpoint Setup (PHP / Laravel / Node)</span>
                  </h4>
                  <div>
                    <label className="block text-[11px] text-slate-700 font-semibold mb-1">Products API Endpoint URL (GET)</label>
                    <input
                      type="url"
                  ...
                  <div>
                    <label className="block text-[11px] text-slate-700 font-semibold mb-1">Order Placed Webhook Endpoint (POST)</label>
                    <input
                      type="url"
                  ...
                  <div>
                    <label className="block text-[11px] text-slate-700 font-semibold mb-1">Lead Captured Webhook Endpoint (POST)</label>
                    <input
                      type="url"
                  ...
                  <div>
                    <label className="block text-[11px] text-slate-700 font-semibold mb-1">API Secret Token (Authorization Bearer Key)</label>
                    <input
                      type="password"
                      placeholder="bearer_token_..."
                      value={customApiToken}
                      onChange={(e) => setCustomApiToken(e.target.value)}
                      className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-900 outline-none font-mono"
                    />
                  </div>
                </div>
              )}
          ...
          {activeTab === "notifications" && (
            <div className="space-y-4">
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                ...
                <div>
                  <label className="block text-[11px] text-slate-700 mb-1">Store Owner WhatsApp Number (with Country Code)</label>
                  <input
                    type="tel"
          ...
          <div className="pt-4 border-t border-slate-200 flex items-center justify-between">
            ...
            <button
              type="submit"
              className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs shadow-md shadow-purple-600/20 transition-all"
            >
              {isSaved ? (
                <>
